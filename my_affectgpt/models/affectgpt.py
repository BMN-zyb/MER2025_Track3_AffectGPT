...
from my_affectgpt.models.mamba import MambaConfig, CrossModalMamba
import config
...
class AffectGPT(Blip2Base):
    ...
    def init_video_Qformer(cls, num_query_token, vision_width, num_hidden_layers=2):
        encoder_config = BertConfig.from_pretrained("/disk3/model/bert-base-uncased")
        encoder_config.num_hidden_layers = num_hidden_layers
        encoder_config.encoder_width = vision_width # 视觉特征维度
        # insert cross-attention layer every other block 每层添加交叉注意力
    ...
    def __init__(
        ...
        print("llama_tokenizer.vocab_size", self.llama_tokenizer.vocab_size)
        ...
        print("特殊token", self.IMAGE_PATCH_TOKEN_ID, self.AUDIO_PATCH_TOKEN_ID, self.FRAME_PATCH_TOKEN_ID, self.FACE_PATCH_TOKEN_ID, self.MULTI_PATCH_TOKEN_ID)
        ...
        self.visual_encoder = registry.get_visual_encoder_class(visual_encoder_name)()  # 从注册器获取视觉编码器
        ...
        if self.video_fusion_type == 'qformer':
            print('!!!Using Video Q-Former!!!')
            self.video_frame_position_embedding = nn.Embedding(32, self.visual_encoder.hidden_size) # [32, featdim] # 视频帧位置编码
            # 初始化视频Q-Former
            print('1=============================================================')
            self.video_Qformer, self.video_query_tokens = self.init_video_Qformer(num_query_token=num_video_query_token,
                                                                                vision_width=self.visual_encoder.hidden_size,  # 视觉特征维度
                                                                                num_hidden_layers=2)  # Q-Former的层数
            print('2=============================================================')
            
            # 移除Q-Former中不必要的模块
            self.video_Qformer.cls = None  # 移除分类器
            self.video_Qformer.bert.embeddings.word_embeddings = None  # 移除词嵌入层
            self.video_Qformer.bert.embeddings.position_embeddings = None  # 移除位置嵌入层
        ...
        ## case2: mean 平均池化
            ...
            print('!!!Using Video Mean Pooling!!!')
            video_hidden_size = self.visual_encoder.hidden_size
        ## case3: attention 注意力池化
        elif self.video_fusion_type == 'attention':
            print('!!!Using Video Attention Pooling!!!')
        ...
        if self.audio_fusion_type == 'qformer':
            print('!!!Using Audio Q-Former!!!')
            self.audio_position_embedding = nn.Embedding(32, self.acoustic_encoder.hidden_size)
        ...
        if self.multi_fusion_type == 'qformer':
            print('!!!Using Multi Q-Former!!!')
            encoder_config = BertConfig.from_pretrained("/disk3/model/bert-base-uncased") # Q-Former 的输出维度是固定的 = 768
            self.multi_position_embedding = nn.Embedding(32, self.max_hidden_size)
        ...
        ## 类型2: 注意力融合
            ...
            print('!!!Using Multi Attention Pooling!!!')
            self.attention_mlp = nn.Linear(self.max_hidden_size * 2, self.max_hidden_size)  # [audio, video] -> [hidden_size]
            self.fc_att = nn.Linear(self.max_hidden_size, 2)  # [hidden_size] -> [2] (音频和视频的注意力权重)
            multi_hidden_size = self.max_hidden_size # 设置 output_hidden_size

        ## 类型3: cross-modal Mamba
        elif self.multi_fusion_type == 'crossMamba':
            print('!!!Using Cross Modal Mamba!!!')
            # 使用 MambaConfig 配置
            mamba_config = MambaConfig(d_model=self.max_hidden_size, n_layers=1, dt_rank='auto', d_state=32, expand_factor=2, d_conv=4, inner_layernorms=True,)
            self.multi_mamba = CrossModalMamba(mamba_config)
            print("frozen_multi_CMamba-----------------------------", frozen_multi_CMamba)
            if frozen_multi_CMamba:
                for name, param in self.multi_mamba.named_parameters():
                    param.requires_grad = False
                print('freeze: multi_CMamba')
            else:
                for name, param in self.multi_mamba.named_parameters():
                    param.requires_grad = True
                print('trainable: multi_CMamba')
    ...
    def encode_image_token(self, image, raw_image):
        device = image.device
        with self.maybe_autocast():  # 自动混合精度上下文
    ...
    # 方法2: 将所有图像特征压缩到1个token     将所有 image 压缩到 1 tokens
    ...
    # 方法1: 使用Q-Former压缩视频特征，将视频的时间维度压缩到 32 tokens
    def encode_video_qformer(self, video, raw_video):
        ...
        with self.maybe_autocast():
            ...
            batch_size, time_length = frame_hidden_state.size()[:2]
            # print(f"$$$$$$$$$$$$$$$$$ encode_video_qformer { batch_size, time_length, frame_hidden_state.size()} $$$$$$$$$$$$$$$$$$")
            ...
            if len(frame_hidden_state.size()) == 4:
                ...
                position_ids = position_ids.unsqueeze(0).expand(batch_size, -1) # [b, t] expand是为了将 position_ids 扩展到 batch size
            ...
            # case2: 输入维度为 [b, t, 768]
                ...
                frame_hidden_state = frame_position_embeddings + frame_hidden_state # [b, t, 768]
            ...
            video_query_tokens = self.video_query_tokens.expand(frame_hidden_state.shape[0], -1, -1)  # expand是为了将 query tokens 扩展到 batch size
            # print(f"$$$$$$$$$$$$$$$$$ video_query_tokens {video_query_tokens.size()} $$$$$$$$$$$$$$$$$$")
            ...
            # print(f"$$$$$$$$$$$$$$$$$ inputs_llama {inputs_llama.size()} $$$$$$$$$$$$$$$$$$")

            # # 检查这5 个 tokens 是否一样
            # if torch.equal(inputs_llama[:, 0, :], inputs_llama[:, 1, :]) and torch.equal(inputs_llama[:, 0, :], inputs_llama[:, 2, :]) and \
            #     torch.equal(inputs_llama[:, 0, :], inputs_llama[:, 3, :]) and torch.equal(inputs_llama[:, 0, :], inputs_llama[:, 4, :]):
            #     print("$$$$$$$$$$$$$$$$$ inputs_llama are the same $$$$$$$$$$$$$$$$$$")
            # else:
            #     print("$$$$$$$$$$$$$$$$$ inputs_llama are different $$$$$$$$$$$$$$$$$$")
    ...
    def encode_video_attention(self, video, raw_video):
        ...
        with self.maybe_autocast():
            ...
            hidden_state = self.visual_encoder(video, raw_video).to(device)
            batch_size, time_length = hidden_state.size()[:2]
            print(f"$$$$$$$$$$$$$$$$$ { batch_size, time_length, hidden_state.size()} $$$$$$$$$$$$$$$$$$")
    ...
    # 方法1: 使用Q-Former压缩音频特征
    def encode_audio_qformer(self, audio, raw_audio):
        device = audio.device
        with self.maybe_autocast():
            
            # print("$$$$$$$$$$$$$$$$$ encode_audio_qformer $$$$$$$$$$$$$$$$$$")
            ...
            # print(f"audio_hidden_state shape: {audio_hidden_state.size()}")
            # print(f"audio_hidden_state device: {audio_hidden_state.device}")
    ...
    def encode_multi_qformer(self, video_hidden_state, audio_hidden_state):
        ...
        with self.maybe_autocast():
            ...
            print(f"video_hidden_state shape: {video_hidden_state.size()}")
            print(f"audio_hidden_state shape: {audio_hidden_state.size()}")
            ...
            print(f"multi_hidden_state shape: {multi_hidden_state.size()}")
            ...
            print(f"inputs_llama shape: {inputs_llama.size()}")
    ...
    def encoder_multi_crossMamba(self, video_hidden_state, audio_hidden_state):
        # print(f'audio_hidden_state shape: {audio_hidden_state.size()}')
        # print(f'video_hidden_state shape: {video_hidden_state.size()}')
        # print ('fusion type: crossMamba')
        device = video_hidden_state.device
        with self.maybe_autocast():

            # print("$$$$$$$$$$$$$$$$$ encoder_multi_crossMamba $$$$$$$$$$$$$$$$$$")
            ...
            # 通过 Mamba 处理特征
            audio_hidden_state, video_hidden_state = self.multi_mamba(audio_hidden_state, video_hidden_state)  # [b, t, maxdim], [b, t, maxdim]
            # print(f"audio_hidden_state shape after mamba: {audio_hidden_state.size()}")
            # print(f"video_hidden_state shape after mamba: {video_hidden_state.size()}")

            # token级 拼接特征 [b, 2, t, maxdim]
            multi_hidden = torch.stack([video_hidden_state, audio_hidden_state], dim=1) # [b, 2, t, maxdim]
            # print(f"multi_hidden shape: {multi_hidden.size()}")

            # token级 平均池化 [b, t, maxdim]
            multi_hidden = torch.mean(multi_hidden, dim=1) # [b, t, maxdim]
            # print(f"multi_hidden shape after mean: {multi_hidden.size()}")

            # + multi_llama_proj
            inputs_llama = self.multi_llama_proj(multi_hidden) # [b, t, llmdim]
            # print(f"inputs_llama shape: {inputs_llama.size()}")

            # # 检查这5 个 tokens 是否一样
            # if torch.equal(inputs_llama[:, 0, :], inputs_llama[:, 1, :]) or torch.equal(inputs_llama[:, 0, :], inputs_llama[:, 2, :]) or \
            #     torch.equal(inputs_llama[:, 0, :], inputs_llama[:, 3, :]) or torch.equal(inputs_llama[:, 0, :], inputs_llama[:, 4, :]):
            #     print("$$$$$$$$$$$$$$$$$ inputs_llama are the same $$$$$$$$$$$$$$$$$$")
            # else:
            #     print("$$$$$$$$$$$$$$$$$ inputs_llama are different $$$$$$$$$$$$$$$$$$")
    ...
    def encode_multi_merge(self, video_hidden_state, audio_hidden_state):
        if self.multi_fusion_type == 'qformer':
            ...
            multi_hiddens, multi_llms = self.encoder_multi_crossMamba(video_hidden_state, audio_hidden_state)
    ...
    def forward(self, samples):
        ...
        if 'images' in samples: 
            ...
            print(f'image_hiddens: {image_hiddens.size()}, image_llms: {image_llms.size()}, images：{samples["images"].size()}, raw_images：{samples["raw_images"].size()}')
        ...
        if 'faces'  in samples: 
            ...
            print(f'face_hiddens: {face_hiddens.size()}, face_llms: {face_llms.size()}, {samples["faces"].size()}, {samples["raw_faces"].size()}')
        ...
        if 'frames' in samples: 
            ...
            print(f'frame_hiddens: {frame_hiddens.size()}, frame_llms: {frame_llms.size()}, frames：{samples["frames"].size()}, raw_frames：{samples["raw_frames"].size()}\n\n') 
            # frame_hiddens: torch.Size([1, 8, 1152]), frame_llms: torch.Size([1, 1, 3584]), torch.Size([1, 3, 8, 384, 384]), torch.Size([1, 3, 8, 224, 224])

        if 'audios' in samples: 
            audio_hiddens, audio_llms = self.encode_audio_merge(samples['audios'],  samples['raw_audios'])
            print(f'audio_hiddens: {audio_hiddens.size()}, audio_llms: {audio_llms.size()}, audios：{samples["audios"].size()}, raw_audios：{samples["raw_audios"].size()}\n\n')
            # audio_hiddens: torch.Size([1, 8, 1024]), audio_llms: torch.Size([1, 1, 3584]), torch.Size([1, 8, 1, 128, 204]), torch.Size([1, 8, 1, 32000])
        ...
        if (samples['input_ids'][0] == self.MULTI_PATCH_TOKEN_ID).sum() != 0: # 这是时候才需要 multi
            ...
            print(f'multi_hiddens: {multi_hiddens.size()}, multi_llms: {multi_llms.size()}')
            # multi_hiddens: torch.Size([1, 1152]), multi_llms: torch.Size([1, 1, 3584])
        ...
        # print('self.FRAME_PATCH_TOKEN_ID: ', self.FRAME_PATCH_TOKEN_ID)
        # print('self.FACE_PATCH_TOKEN_ID: ', self.FACE_PATCH_TOKEN_ID)
        # print('self.AUDIO_PATCH_TOKEN_ID: ', self.AUDIO_PATCH_TOKEN_ID)
        # print('self.MULTI_PATCH_TOKEN_ID: ', self.MULTI_PATCH_TOKEN_ID)
        # print('self.IMAGE_PATCH_TOKEN_ID: ', self.IMAGE_PATCH_TOKEN_ID)
        input_ids = samples['input_ids']
        print('\ninput_ids:\n', input_ids, input_ids.size())
        temp_input_ids = copy.deepcopy(input_ids)  # 深拷贝 input_ids，避免修改原始数据, 用于替换特殊 token
        temp_input_ids[temp_input_ids == self.FRAME_PATCH_TOKEN_ID] = 0  # 0是一个占位token，LLaMA 模型的嵌入层会将其映射为零向量
        ...
        cur_idx = 0
        new_input_embeds = []
        for cur_input_ids, cur_input_embeds in zip(input_ids, temp_input_embedding): # 分别表示当前样本的输入 token ID 和对应的嵌入, 次数是 input_ids 的 batch size

            # 包含每种特殊 token 的 ID (patch_token_id)、对应的查询 token 数量 (query_token_number)，以及实际的特征嵌入 (embeds)。
            # 依次检查每种特殊 token 是否存在于当前样本的 cur_input_ids 中。
            for (patch_token_id, query_token_number, embeds) in [(self.FRAME_PATCH_TOKEN_ID, self.num_video_query_token, frame_llms),
                ...
                # 检查当前输入是否包含特定的 patch token，包含则进行替换
                if (cur_input_ids == patch_token_id).sum() != 0:
                    assert embeds is not None, f'Some input info is missing.'
                    cur_features = embeds[cur_idx]  # 获取当前样本的特征嵌入。

                    print('\n\n\n\n\n',
                          '\ncur_idx:', cur_idx,
                          '\nembeds:', embeds.size(),
                          '\ncur_features:', cur_features.size(),
                          '\nembeds == cur_features: ', embeds.eq(cur_features).all(),
                          '\npatch_token_id:', patch_token_id, 
                          '\nquery_token_number:', query_token_number,
                          '\ncur_input_ids:', cur_input_ids,
                          '\ncur_input_embeds:', cur_input_embeds.size())
                    

                    print('cur_input_ids == patch_token_id:', (cur_input_ids == patch_token_id).sum())
                    if (cur_input_ids == patch_token_id).sum() != query_token_number:  # 检查音频/视频/图像 patch token 数量是否正确
                    ...
                    # 检查替换后的 cur_input_embeds 是否正确
                    for i in range(mask_index_start, mask_index_start + query_token_number):
                        if torch.equal(cur_input_embeds[i], cur_features[i - mask_index_start]):
        ...
        print('==================================')
        print('temp_input_embedding:', temp_input_embedding.size())
    ...
    def from_config(cls, cfg):
        ...
        frozen_multi_CMamba = cfg.get("frozen_multi_CMamba", False) # crossMamba
        ...
        model = cls(
            ...
            frozen_multi_CMamba=frozen_multi_CMamba,  # crossMamba
