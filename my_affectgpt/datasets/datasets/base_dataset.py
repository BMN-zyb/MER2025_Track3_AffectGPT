...
class BaseDataset():
    def __init__(self, vis_processor=None, txt_processor=None, img_processor=None, model_cfg=None, dataset_cfg=None,
        ...
        # sample4 = self.__getitem__(random.randint(0, len(self)-1))
        # sample5 = self.__getitem__(random.randint(0, len(self)-1))
        ...
        # print('\n\n\n')
        # self.func_visualize_samples(sample4)

        # print('\n\n\n')
        # self.func_visualize_samples(sample5)

        samples = [sample1, sample2, sample3]
        self.collater(samples)  # collater 函数会检查数据的完整性和正确性
    ...
    def _get_max_token_length(self):
        '''
        找到当前数据集中最大的 token 长度
        '''
        import tqdm
        max_length = 0
        print(f'++++++++++++++++++++ Calculating maximum token length for dataset {self.dataset}... ++++++++++++++++++++')
        for index in tqdm.tqdm(range(len(self))):
            sample = self.__getitem__(index)
            text_input = sample['label']
            if len(text_input) > max_length:
                max_length = len(text_input)
            print(f'index: {index}, text_input length: {len(text_input)}')

        print(f'Maximum token length in dataset {self.dataset} is {max_length}.')
        return max_length
    ...
    def func_visualize_samples(self, sample):
        ...
        print('ignore index: ', config.IGNORE_INDEX, '\n', self.tokenizer.bos_token_id, '\n')
        label = copy.deepcopy(sample['label'])  # 这里的 label 是一个 tensor
        label[label==config.IGNORE_INDEX] = self.tokenizer.bos_token_id  # 将 ignore_index 替换为 bos_token_id
        output_convert = self.tokenizer.decode(label)
        print('label: ', label, '\n')
        print (output_convert, '\n\n')
    
    # to_token_ids: 开头不增加特殊符号，裁剪输入保证不超过 max_length，max_length 是指输入的最大长度
    ...
    def read_frame_face_audio_text(self, video_path=None, face_npy=None, audio_path=None, image_path=None):
        ...
        if audio_path is not None and 'audio' in self.needed_data:
            raw_audio = load_audio([audio_path], "cpu", clips_per_video=self.n_frms)[0] # [8, 1, 16000*2s]
    ...
    def func_get_qa_description(self, sample, question_only=False):
        ...
        question = "Please consider the following points: observe facial expressions and body movements, evaluate speech rate, tone, and volume, and analyze what the character says. " \
            "Finally, infer the person's emotional state and provide your reasoning process."
    ...
    def get_qa_pairs(self, dataset, label_type, sample):
        ...
        elif dataset in ['MERCaptionPlus', 'OVMERD', 'MSA']:  # 添加 MSA 数据集 =====================
            candidates = {
                'description': self.func_get_qa_description(sample),
                'ovlabel':     self.func_get_qa_ovlabel(sample),
            }
