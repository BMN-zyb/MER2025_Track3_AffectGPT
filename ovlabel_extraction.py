...
if __name__ == '__main__':
    ...
    result_path = '/disk3/MER-2025/MER2025_Track23/output/results-description-mer2025ov/msa_outputhybird_bestsetup_bestfusion_frame_lz_20250705141/checkpoint_000005_loss_0.952.npz'
    print('Processing:===========================================', result_path)
    # only process for result_path files
    modelname = result_path.split('/')[-2]
    if result_path.endswith('-openset.npz'):
        print('Skip processing for openset file:', result_path)
    else:
        # reason -> openset
        openset_npz = result_path[:-4] + '-openset.npz'
        if not os.path.exists(openset_npz):
            ...
            print('Skip processing for existing openset file:', openset_npz)
    ...
    npz_file_path = '/disk3/MER-2025/MER2025_Track23/output/results-description-mer2025ov/msa_outputhybird_bestsetup_bestfusion_frame_lz_20250705141/checkpoint_000005_loss_0.952-openset.npz'
    ...
    with np.load(npz_file_path, allow_pickle=True) as data:
        ...
        # 组合数据并创建DataFrame
        data = np.column_stack([filenames, processed_labels])
        df = pd.DataFrame(data=data, columns=['name', 'openset'])

    save_path = '/disk3/MER-2025/MER2025_Track23/output/results-description-mer2025ov/msa_outputhybird_bestsetup_bestfusion_frame_lz_20250705141/checkpoint_000005_loss_0.952-openset.csv'
